...
for (const ex of excelRows) {
    if (!dbByCode.has(ex.code))
        missingCodes.push(ex.code);
}
...
for (const ex of excelRows) {
    ...
    if (ex.hra > 0) {
        data.push({
            ...
            percentageValue: ex.hraPct?.pct ?? Math.round(((ex.hra / ex.basic) * 100) * 100) / 100,
        });
    }
    if (ex.spec > 0) {
        data.push({
            ...
            percentageValue: ex.specPct?.pct ?? Math.round(((ex.spec / ex.basic) * 100) * 100) / 100,
        });
    }
    if (ex.pf > 0) {
        data.push({
...
let hraDrift = 0, specDrift = 0, grossDrift = 0, pfDrift = 0, esicDrift = 0, emprPfDrift = 0, emprEsDrift = 0, ctcDrift = 0;
const details = [];
for (const ex of excelRows) {
    ...
    if (specC !== ex.spec)
        specDrift++;
    if (Math.abs(grossC - ex.gross) > 1)
        grossDrift++;
    if (pfC !== ex.pf)
        pfDrift++;
    if (esicC !== ex.esic)
        esicDrift++;
    if (emprPfC !== ex.emprPf)
        emprPfDrift++;
    if (emprEsC !== ex.emprEs)
        emprEsDrift++;
    if (Math.abs(ctcC - ex.ctc) > 2)
        ctcDrift++;
    if (hraC !== ex.hra ||
        specC !== ex.spec ||
        pfC !== ex.pf ||
