const getStartEndOfDay = (timeZone = "Asia/Kolkata", inputDate) => {
    ...
    const startLocalUTC = Date.UTC(Number(map.year), Number(map.month) - 1, Number(map.day), 0, 0, 0, 0);
    // 5️⃣ End of day (timezone)
    const endLocalUTC = Date.UTC(Number(map.year), Number(map.month) - 1, Number(map.day), 23, 59, 59, 999);
