import { adminMarkAttendanceService, getAttendanceByRange, getCompanyDayAttendance, getMonthlyAttendance, getMonthlyAttendanceAll, getTodayAttendance, getUserlessAttendanceService, handleAttendance, } from "../services/handleAttendance/attendance.service.js";
...
export const getUserlessAttendanceController = async (req, res) => {
    try {
...
//   }
// };
export const getCompanyDay = async (req, res) => {
    try {
        //    const employee = req.employee;
        ...
        const data = await getCompanyDayAttendance(Number(req.companyId), // or employee.companyId
        parsedDate);
        res.json({
            success: true,
            data,
...
export const getByRange = async (req, res) => {
    try {
        ...
        const data = await getAttendanceByRange(Number(companyId), Number(employeeId), new Date(startDate), new Date(endDate));
        res.json({
            success: true,
            data,
        });
...
//   companyId?: number;
// }
export const getTodayAttendanceByEmployee = async (req, res) => {
    try {
        // ======================================
        // TODAY RANGE
        // ======================================
...
export const getEmployeeMonthlyAttendances = async (req, res) => {
    try {
        const employee = req.employee;
        if (!employee)
            throw new Error("Employee not found");
        ...
        const { start, end } = getStartEndOfMonth("Asia/Kolkata", Number(year), Number(month));
...
export const getMonthlyAttendanceController = async (req, res) => {
    try {
        ...
        const data = await getMonthlyAttendance(Number(req.companyId), employee.id, Number(year), Number(month));
        res.json({
            success: true,
            data,
        });
...
export const getMonthlyAttendanceAllController = async (req, res) => {
    try {
        // const employee = req.employee;
        // if (!employee) throw new Error("Employee not found");
        // console.log("request:", req.companyId, employee.id);
        ...
        const data = await getMonthlyAttendanceAll(Number(req.companyId), Number(year), Number(month));
        res.json({
            success: true,
            data,
        });
