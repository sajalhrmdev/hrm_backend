...
//   return adjustments;
// };
import { AttendanceAdjustmentType, AttendanceStatus, } from "../../generated/prisma/enums.js";
import { prisma } from "../../lib/prisma.js";
...
export const regularizeAttendance = async (input) => {
    ...
    let totalMinutes = attendance.total_work_minutes;
    if (updatedCheckIn && updatedCheckOut) {
        totalMinutes = Math.floor((updatedCheckOut.getTime() - updatedCheckIn.getTime()) / (1000 * 60));
    }
    // ======================================================
    // EFFECTIVE WORK
    // ======================================================
    ...
    if (!isFlexible &&
        ...
        const [startHour, startMinute] = shift.startTime.split(":").map(Number);
        const shiftStart = new Date(updatedCheckIn);
        shiftStart.setHours(startHour, startMinute + (shift.graceMinutes || 0) + (shift.lateAfterMinutes || 0), 0, 0);
        if (updatedCheckIn > shiftStart) {
            lateMinutes = Math.floor((updatedCheckIn.getTime() - shiftStart.getTime()) / (1000 * 60));
        }
        else {
            lateMinutes = 0;
        }
    ...
    // EFFECTIVE LATE
    // ======================================================
    const finalLateMinutes = Math.max(0, lateMinutes - lateGraceMinutes);
    // ======================================================
    // OVERTIME
    // ======================================================
    let overtime = 0;
    if (isFlexible) {
        overtime = overTimeCalculation4Nonshift(totalMinutes, attendance.employee.workSchedulePolicy, overtime).overtime;
    }
    else {
        overtime = overTimeCalculation4Shift(totalMinutes, attendance.employee.workSchedulePolicy?.shift, overtime).overtime;
    }
    // ======================================================
    // AUTO STATUS
    // ======================================================
    ...
    if (isFlexible) {
        ...
        autoStatus = attendanceStatusFn(effectiveWorkMinutes, policy, attendance.employee.workSchedulePolicy?.shift).status;
    }
    // ======================================================
    // FINAL STATUS
    // ======================================================
...
// ======================================================
export const getCompanyAdjustmentByDay = async (input) => {
    const { companyId, date } = input;
    // ======================================================
    // DATE RANGE
    // ======================================================
...
// ======================================================
export const getAttendanceAdjustments = async (input) => {
    const { companyId, attendanceId, } = input;
    const adjustments = await prisma.attendanceAdjustment.findMany({
        where: {
            companyId,
            attendanceId,
