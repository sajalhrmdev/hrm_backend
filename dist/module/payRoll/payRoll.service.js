...
export const createPayrollRun = async (input) => {
    const { companyId, periodStart, periodEnd, title, } = input;
    // ========================================
    // VALIDATION
    // ========================================
...
export const getAllPayrollRuns = async (companyId) => {
...
// GENERATE PAYROLL
// ============================================
export const generatePayroll = async (companyId, payrollRunId) => {
    // GET PAYROLL RUN----------
    const payrollRun = await prisma.payRollRun.findFirst({
        where: {
            id: payrollRunId,
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        for (const employee of employees) {
            ...
            const holidayDays = attendanceMap.get(`${employee.id}_HOLIDAY`)?.count || 0;
            ...
            const totalWorkMinutes = aggregateMap.get(employee.id)?.workMinutes || 0;
...
export const getPayrollsByRunId = async (companyId, payrollRunId) => {
    // ============================================
    // CHECK PAYROLL RUN
    // ============================================
    const payrollRun = await prisma.payRollRun.findFirst({
...
// 5=============================getSinglePayroll============================
export const getSinglePayroll = async (companyId, payrollId) => {
    // ============================================
    // GET PAYROLL
    // ============================================
...
export const finalizePayrollRun = async (companyId, payrollRunId) => {
    // ============================================
    // FIND PAYROLL RUN
    // ============================================
    const payrollRun = await prisma.payRollRun.findFirst({
...
// 7==============================MarkPayrollPaid=========================
export const markPayrollPaid = async (companyId, payrollId) => {
    // ============================================
    // FIND PAYROLL
    // ============================================
    const payroll = await prisma.payRoll.findFirst({
...
export const markPayrollRunPaid = async (companyId, payrollRunId) => {
    // ============================================
    // FIND PAYROLL RUN
    // ============================================
    const payrollRun = await prisma.payRollRun.findFirst({
    ...
    // ============================================
    const unpaidPayrolls = payrollRun.payrolls.filter((item) => item.status !== "PAID");
