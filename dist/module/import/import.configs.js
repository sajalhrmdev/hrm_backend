...
export const importConfigs = [
    ...
    // ============================================
    {
        ...
        instructions: "Resolve each employee by EMPLOYEE CODE (or Employee Email as fallback).\n" +
...
// ============================================
async function salaryComponentRowValidator(data, companyId) {
    const errors = [];
    ...
    if (calculationType === "PERCENTAGE") {
        ...
        if (!baseType) {
            errors.push({
                row: 0,
                field: "Base Type",
                message: "Percentage component requires a base type (COMPONENT, COMPONENTS or GROSS)",
            });
        }
            ...
            if (!data.baseComponentId) {
                ...
                else if (String(base.code).toLowerCase().trim() ===
            ...
            const ids = Array.isArray(data.baseComponentIds)
                ? data.baseComponentIds.map((n) => Number(n)).filter((n) => !Number.isNaN(n))
                : [];
            if (!ids.length) {
                errors.push({
                    row: 0,
                ...
                if (found.some((b) => String(b.code).toLowerCase().trim() ===
                    String(data.code ?? "").toLowerCase().trim())) {
                    errors.push({
                        row: 0,
                        field: "Base Components",
...
// ============================================
async function employeeSalaryComponentRowValidator(data, companyId) {
    const errors = [];
    ...
    const calculationType = data.calculationType ?? component.calculationType ?? "FIXED";
    ...
    if (calculationType === "PERCENTAGE") {
        ...
        const percentageValue = data.percentageValue ?? component.percentageValue ?? null;
        if (percentageValue == null || percentageValue <= 0) {
            errors.push({
                row: 0,
                field: "Percentage Value",
...
;
export function getImportConfig(entity) {
    return importConfigs.find((c) => c.entity === entity);
}
export function getAllImportConfigs() {
