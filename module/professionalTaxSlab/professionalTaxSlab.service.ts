...
export const createSlabService = async (
  companyId: number,
  body: { minSalary: number; maxSalary?: number | null; taxAmount: number }
) => {
  ...
  if (existing) throw new Error("Slab with this minSalary already exists");
...
export const getAllSlabsService = async (companyId: number) => {
...
export const updateSlabService = async (
  id: number,
  companyId: number,
  body: { minSalary?: number; maxSalary?: number | null; taxAmount?: number }
) => {
  ...
  if (body.minSalary !== undefined && body.minSalary !== slab.minSalary) {
    ...
    if (dup) throw new Error("Slab with this minSalary already exists");
  }
...
export const deleteSlabService = async (id: number, companyId: number) => {
  const slab = await prisma.professionalTaxSlab.findFirst({
    where: { id, companyId },
  });
...
export const findApplicableSlab = async (
  companyId: number,
  netSalary: number
) => {
