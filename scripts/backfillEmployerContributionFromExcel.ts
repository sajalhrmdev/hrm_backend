...
const normalizeCode = (c: string): string | null => {
...
const excelMap = new Map<string, { pf: number; es: number; name: string }>();
const noCodeMap = new Map<string, { pf: number; es: number }>();
...
const matchFor = (p: (typeof payrolls)[number]) => {
  ...
  if (byName) return { pf: byName.pf, es: byName.es, via: "name" };
