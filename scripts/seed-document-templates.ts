...
const HEADER = (companyName: string) => `
...
const LABEL = (text: string) => `color:#aaa;font-size:9px;text-transform:uppercase;letter-spacing:1.5px;font-weight:400;`;
...
const seed = async () => {
  ...
  if (argId && argId !== "all") {
    ...
    if (!all.length) { console.log("No companies found."); await prisma.$disconnect(); return; }
  ...
  for (const companyId of companyIds) {
    ...
    for (const tpl of templates) {
      try {
        ...
        if (existing) {
          await prisma.documentTemplate.update({ where: { id: existing.id }, data: { name: tpl.name, subject: tpl.subject, htmlContent: tpl.htmlContent, description: tpl.description, category: tpl.category as any } });
          console.log(`  Updated: ${tpl.name}`);
        } else {
          await prisma.documentTemplate.create({ data: { companyId, name: tpl.name, slug: tpl.slug, category: tpl.category as any, subject: tpl.subject, htmlContent: tpl.htmlContent, description: tpl.description, isActive: true } });
